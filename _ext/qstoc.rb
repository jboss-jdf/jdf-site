          ...
          if page.relative_source_path =~ /^#{@path_prefix}\/[^\/]+$/ && page.rendered_content
            ...
            if site.guides[@path_prefix]
              site.guides[@path_prefix].each do |guide|
                if guide.dir
                  qs_toc_content += "<tr><td align='left'><a href='#{site.base_url}#{guide.url}' title='#{guide.dir}'>#{guide.dir}</td><td align='left'>#{split(guide.technologies)}</td><td align='left'>#{guide.summary}</td><td align='left'>#{guide.level}</td><td align='left'>#{split(guide.prerequisites)}</td></tr>"
                end
              end
