          ...
          if page.relative_source_path =~ /^#{@path_prefix}\/[^\/]+$/ && page.rendered_content
            ...
            if site.guides[@path_prefix]
              site.guides[@path_prefix].each do |guide|
                qs_toc_content += "<tr><td align='left'><a href='#{guide.dir}' title='#{guide.dir}'>#{guide.dir}</td><td align='left'>#{split(guide.technologies)}</td><td align='left'>#{split(guide.summary)}</td><td align='left'>#{split(guide.level)}</td><td align='left'>#{split(guide.prerequisites)}</td></tr>"
              end
